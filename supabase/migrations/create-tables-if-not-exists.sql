...
-- Create commands table for system commands (if not exists)
...
-- Create system_status table for device monitoring (if not exists)
...
DROP POLICY IF EXISTS "Users can view their own system status" ON public.system_status;
DROP POLICY IF EXISTS "Users can create their own system status" ON public.system_status;
DROP POLICY IF EXISTS "Users can update their own system status" ON public.system_status;
...
-- Create function to handle new user profiles (replace if exists)
...
CREATE INDEX IF NOT EXISTS idx_system_status_user_id ON public.system_status(user_id); 